...
## Cross-Cutting: Two Parallel Application Stacks

This finding cuts across every section below and is stated up front because it reframes how several of the per-section recommendations (especially Section 3's) should be read.

**The mobile app and the Node/Express backend are two entirely separate applications with separate databases.** The mobile app (`mobile/`) talks exclusively to Supabase — its own Postgres instance, reached through the Supabase JS client (`mobile/src/services/supabase.js`) and Supabase's own Auth, Realtime, and Edge Functions products. The Node/Express backend (`server.js` + the route modules at the repo root) runs its own, entirely separate Postgres database, defined and migrated by `db.js`. Nothing at runtime connects the two: mobile never calls the Node API, and the Node backend has no Supabase client or credentials anywhere in it.

**Evidence:**
- `grep -rn "/api/" mobile/src` returns zero hits — the mobile app never calls the Node backend's HTTP API (`/api/...` routes) anywhere in its source. Its only data client is `mobile/src/services/supabase.js`, and all of `mobile/src/services/api.js`'s 56 exported functions (see Section 4) go through the Supabase client, not `fetch()` against the Node backend.
- `public/preview-b/app.js` is the only consumer of the Node API in the repo: `/api/auth/login` (lines 76, 329), `/api/auth/register` (line 291), `/api/match/profiles` (line 435), `/api/signup` (line 696), `/api/cafe/hyperbeam/create` (line 730), and others (e.g. `/api/messages/:id` at line 875). Conversely, `grep -ci supabase public/preview-b/app.js public/app.js public/app-auth.js` returns `0` in all three files — the web frontend (Section 2) never touches Supabase. The web frontend is a client of the Node backend; the mobile app is a client of Supabase. They do not overlap.
- `db.js` (lines 37-171) creates Postgres tables `users, profiles, messages, reports, blocks, prompts, likes, matches, interests, passes, sessions, signups, cafe_rooms`. Mobile instead reads/writes Supabase tables `profiles, matches, messages, swipes, reports, blocks, prompts, interests, cafe_rooms, cafe_participants, cafe_messages, profile_photos, video_calls` (verified via `grep -rn "\.from('" mobile/src/services/*.js`). The table sets overlap in name for several entities but diverge in others: Node's `likes`/`passes` correspond to Supabase's single `swipes` table; `sessions` and `signups` are Postgres-only (no mobile equivalent); `profile_photos`, `video_calls`, `cafe_participants`, and `cafe_messages` are Supabase-only (no Node equivalent). These are two independently-evolved schemas, not one schema accessed two ways.
- The two stacks each have their own auth system (Node: bcrypt password hashing + a custom `sessions` token table, see `auth.js`/`db.js`; mobile: Supabase Auth, see `services/supabase.js` and `initAuth()` in `services/api.js`), their own realtime system (Node: a `ws` WebSocket server, `websocket.js`; mobile: Supabase Realtime channel subscriptions, e.g. `subscribeToCafeRoom`/`subscribeToMessages`/`subscribeToVideoCalls` in `services/api.js`), and their own video/room provider (Node's `cafe.js` calls Hyperbeam for the web frontend's café rooms, see Section 5's addendum below; mobile calls Daily.co directly via the Supabase edge function, see Section 5).

**Tag:** consolidate

**Recommendation:** Before acting on Section 3's backend-restructuring recommendations, the user should decide the Node backend's status: is it still the intended production backend going forward, or has mobile — the actively-developed client, per Section 2's evidence that `mobile/` saw commits as recently as 2026-08-03 versus `public/`'s last commit on 2026-07-03 — already moved fully to Supabase, leaving the Node stack legacy/web-only? Section 3's 8 findings are all real and independently worth fixing *if* the Node backend continues to be invested in, but that investment is a decision this audit surfaced rather than one it can make; restructuring `db.js`/`auth.js`/etc. before that decision risks polishing a stack that may be scoped down to "whatever keeps `public/preview-b/` running" rather than a shared backend.
...
The 8 findings below apply to the Node backend specifically — not the mobile app, which does not use this backend at all (see "Cross-Cutting: Two Parallel Application Stacks" above). Whether it's worth acting on them is conditional on the decision that cross-cutting finding raises: they assume ongoing investment in this backend as a stack worth structurally improving.
...
| `message.js`, `profile.js`, `safety.js`, `cafe.js`, `admin.js`, `reports.js` | Same routing+raw-SQL pattern as `auth.js`/`match.js` is present at smaller scale in every other route module (e.g. `profile.js` lines 33, 44, 53, 59, 61, 78, 93, 96; `cafe.js` lines 11-17, 27-30; `admin.js` lines 26, 35, 45, 47-50, 60-64, 73-77). Each file is small and internally cohesive, so this is a repo-wide architectural convention rather than a per-file defect. **Additional dimension in `cafe.js` specifically:** beyond routing+SQL, `cafe.js` also mixes in a third-party HTTP integration — its `POST /hyperbeam/create` route (lines 38-54) holds the `HYPERBEAM_API_KEY` secret and calls `https://engine.hyperbeam.com/v0/vm` directly from inside the route handler. `grep -rn -i hyperbeam mobile/src` returns no hits, confirming this route has no mobile consumer — it exists solely to serve `public/preview-b/app.js`'s café/movie/game rooms (see Section 2 and Section 5's addendum below). So `cafe.js` mixes three responsibilities in 55 lines: HTTP routing, raw SQL, and third-party API-secret-holding integration code. | restructure | Not urgent to change file-by-file given their size, but if the `auth.js`/`match.js` extraction above is done, apply the same `db.js` query-function pattern here for consistency rather than leaving two conventions (raw SQL in small files, extracted functions in large ones). For `cafe.js` specifically, also consider moving the Hyperbeam call into its own small integration module (e.g. `hyperbeam.js`), matching the separation already recommended for `db.js`. |
...
No finding — cleanly integrated, scoped to the Daily.co flow. Step 2 shows a single, clear call site (`createVideoCall()` in `mobile/src/services/api.js`, invoking the edge function directly via `supabase.functions.invoke('create-video-call', ...)`), and Step 3 shows no duplicated Daily.co room-creation logic: the edge function is the only place that holds `DAILY_API_KEY` and calls Daily's `/v1/rooms` and `/v1/meeting-tokens` REST endpoints, while the two other Daily-related hits are (a) plain `video_calls` table reads/writes/subscriptions with no Daily API calls, and (b) the Daily *client* SDK in `VideoCallScreen.js` joining the room the edge function already built — a legitimate, non-duplicative division of labor (server-side secret-holding room creation in the edge function; client-side call-join UI in the mobile app). The Node/Express backend is **not involved in the Daily.co video-call flow specifically** — not as an orphaned parallel path, just genuinely uninvolved, since the mobile app talks to the edge function directly.

**Scope note:** this step's search was `grep -rln "daily\|Daily"` against `server.js *.js mobile/src` — deliberately targeted at the Daily.co vendor name and excluding `public/`. It does not mean the Node backend has no video/co-browsing involvement at all: `cafe.js`'s `POST /hyperbeam/create` route (see Section 3's `cafe.js` finding above) is a real, different-vendor video/co-browsing integration — it holds `HYPERBEAM_API_KEY` and calls `https://engine.hyperbeam.com/v0/vm` — used by `public/preview-b/app.js`'s café/movie/game rooms (Section 2). `grep -rn -i hyperbeam mobile/src` returns no hits, so this is a Node-backend-and-web-frontend-only integration with no mobile counterpart — the two stacks each ended up with their own, unrelated video/room vendor (see "Cross-Cutting: Two Parallel Application Stacks" above).

## 6. Stack Consistency

- **File/Directory:** `Intro/`, `app/` (TypeScript) vs. `server.js` + root `*.js` + `mobile/` (JavaScript)
- **Description:** The only TypeScript in the repo lives in the two trees Section 1 identified as dead scaffold (plus the Supabase edge function at `supabase/functions/create-video-call/index.ts`, which is conventionally TypeScript and not part of this inconsistency). A repo-wide search for `*.ts`/`*.tsx` files outside `Intro/`, `app/`, and `supabase/` returned no results — confirming there is no other TypeScript anywhere in the active codebase. Section 1 recommends removing both `Intro/` and `app/` entirely as dead scaffold, so this finding resolves itself once that removal happens rather than being a separate open question. There is no live *language*-level (TypeScript-vs-JavaScript) mixed-stack problem to solve: the active codebase (backend `server.js` + root `*.js`, and `mobile/`) is consistently JavaScript. Note this section is scoped narrowly to programming language — it does not speak to overall stack coherence. See "Cross-Cutting: Two Parallel Application Stacks" near the top of this report for a real, non-language stack-coherence issue: the mobile app and the Node backend are two disjoint applications on two separate databases.
- **Tag:** dead-code
- **Recommendation:** No action needed beyond finding 1.1/1.2's dead-code removal — resolves this automatically. If any part of `Intro/` or `app/` is ever kept instead of removed, the TS-vs-JS split becomes a decision to make explicitly (e.g. migrate `mobile/` to TypeScript, or port the retained piece back to JS) rather than something to leave ambiguous.
...
There is no Flutter code anywhere in the current repo (no `lib/main.dart`, no `pubspec.yaml`); the Flutter framing in `README.md` reflects an earlier, abandoned concept — consistent with the Flutter/IDE leftover files (`.metadata`, `intro.iml`) flagged as dead code in Section 1, finding 1.4. `replit.md` is eight months newer than `README.md` and accurately describes the live architecture (Node/Express backend + React Native/Expo mobile app), while `README.md` is stale and misleading to anyone using it as an entry point to the project.

### `Intro/AGENTS.md` and `Intro/CLAUDE.md`

Both files live inside `Intro/`, the parallel scaffold directory that Section 1, finding 1.1, recommends removing entirely (tagged `dead-code`, not referenced by any `.replit` workflow or other file in the repo). `Intro/AGENTS.md` and `Intro/CLAUDE.md` were both added in commit `c9d09a2` (2026-06-01, "Published your App"), the same commit series that Section 1 identifies as the automated scaffold-publishing pattern; the `Intro/` directory as a whole was last touched by commit `b26063d` (2026-06-02), also cited in Section 1. No root-level `AGENTS.md`/`CLAUDE.md` exist in this repo for these two files to duplicate or conflict with — they document only the `Intro/` scaffold itself and have no independent reason to exist once that directory is removed.

### `mobile/PHOTO_VIDEO_PLAN.txt`

Already flagged in Section 4, finding 4.3: a 236-line implementation plan, last touched 2026-07-10, whose every specified function and UI handler is already implemented in `mobile/src/services/api.js` and `mobile/src/screens/Profile/ProfileScreen.js`. Finding 4.3 tags it `consolidate` and recommends moving it to `docs/` (or deleting it) since it's no longer actionable and currently sits in a source directory where it could be mistaken for an active spec. See Section 4, finding 4.3 above for the full analysis; not re-litigated here.
...
| `Intro/AGENTS.md`, `Intro/CLAUDE.md` | Live inside `Intro/`, the parallel scaffold directory finding 1.1 recommends removing entirely (unreferenced by any `.replit` workflow or other repo file). Both added in commit `c9d09a2` (2026-06-01), part of the same automated "Published your App" scaffold pattern finding 1.1 documents; `Intro/` as a whole was last touched by commit `b26063d` (2026-06-02). No content in either file is independent of the `Intro/` scaffold it documents. | dead-code | Remove alongside the rest of `Intro/` per finding 1.1's recommendation — no standalone action needed beyond that directory removal. |
| `mobile/PHOTO_VIDEO_PLAN.txt` | See Section 4, finding 4.3 above for full analysis: a 236-line implementation plan, last touched 2026-07-10, fully implemented (every listed function/handler already exists in `services/api.js` and `ProfileScreen.js`), sitting in a source directory rather than `docs/`. | consolidate | See finding 4.3 above — move to `docs/` or delete; not repeated here. |

## Summary

23 individual findings across 8 audit areas (Section 5, the Supabase edge function integration, produced no finding — confirmed genuinely clean). The table below has one row per finding, not per section, since Sections 2, 3, 4, and 7 each contain multiple distinct findings.

| # | Area | File/Directory | Tag | Recommendation |
|---|------|-----------------|-----|-----------------|
| 0.1 | Cross-cutting | Node backend (`db.js`, `auth.js`, etc.) vs. mobile (`mobile/`, Supabase) | consolidate | Decide whether the Node backend is still the intended production backend, or legacy/web-only, before acting on Section 3's findings — see "Cross-Cutting: Two Parallel Application Stacks" near the top of this report. |
