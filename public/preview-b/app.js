document.addEventListener('DOMContentLoaded', () => {
  ...
  // Delegated click wiring for the questionnaire/splash controls that used to be
  // inline onclick="" attributes. Helmet's CSP sets script-src-attr 'none', which
  // silently blocks those attributes, so they're routed through data-action here.
  document.addEventListener('click', (e) => {
    ...
    switch (target.dataset.action) {
      ...
      case 'select-radio': {
        ...
        if (input) window.selectRadioOption(target, input.name);
        break;
      }
      case 'toggle-tag':
        window.toggleTag(target);
        break;
      case 'pick-photo':
